...
1. 下载frp可执行文件:`wget https://github.com/fatedier/frp/releases/download/v0.35.1/frp_0.35.1_linux_amd64.tar.gz`
2. 解压缩文件:`tar -zxf frp_0.35.1_linux_amd64.tar.gz`
3. 将可执行文件复制到**bin**目录下:`sudo cp frps /usr/bin/`
4. 在**etc**创建配置文件夹:`sudo mkdir /etc/frp/`
5. 创建默认配置文件:`sudo cp frps.ini /etc/frp/`
6. 注册frp服务:`sudo cp systemd/frps* /lib/systemd/system/`
7. 设置为开机自动启动:`sudo systemctl enable frps`
8. 启动frp服务:`sudo systemctl start frps`
...
1. 参考的服务器端配置如下:
    ...
    ops = NewProxy
    ```
2. 部署frp服务器端时,应将frps.service文件中的**User=nobody**部分去掉,完整的配置文件如下:
