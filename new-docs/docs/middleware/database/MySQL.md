...
由于一般应用程序的"页"定义都比操作系统文件系统的"页"定义要大,为了防止由于意外情况导致部分本地文件系统的"页"写入失败,因此引入DoubleWriteBuffer机制;

DoubleWriteBuffer本质是一个内存+磁盘的区域,默认情况下内存占用大小为128页即2Mb;

主要作用时用来防止出现在数据从bufferPool写入磁盘时由于意外导致关机等场景产生的数据丢失现象,由

1. BufferPool数据写入内存的DoubleWriteBuffer;
2. 内存的DoubleWriteBuffer追加写入DoubleWriteBuffer本地文件;
3. 内存的DoubleWriteBuffer写入MySQL本地文件;
