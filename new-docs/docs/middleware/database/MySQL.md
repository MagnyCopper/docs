...
Buffer Poll 是MySQL的InnoDB引擎用来在内存中保存从磁盘加载的页数据的,默认大小为128m;

为了保证Buffer Poll的正常更新和淘汰页,有以下3个链表负责维护相关数据:

- free链表: 用于维护当前buffer pool中的可用空间位置;
- flush链表: 用户维护当前buffer pool中的脏页位置;
- lru链表: 包含了热数据区(5/8)和冷数据区(3/8),冷数据区的页被间隔1秒以上再次访问时将升级至热数据区;

当发生数据更新操作时,MySQL将先更新Buffer Pool中的数据并在事务提交后将该操作写入redo log文件,由其他线程定时向本地磁盘文件更新;
